...
typedef void (*VideoStreamRenderEventFinalizeCallback)();
VideoStreamRenderEventFinalizeCallback OnVideoStreamRenderEventFinalize;
#pragma endregion
static void UNITY_INTERFACE_API OnRenderEvent(int eventID, void *data)
{
    if (s_context == nullptr)
    {
        MyDebug::Log("MyDebug::OnRenderEvent:: s_context == nullptr");
        return;
    }
    if (!ContextManager::GetInstance()->Exists(s_context))
    {
        MyDebug::Log("MyDebug::OnRenderEvent:: !GetInstance()->Exists(s_context)");
        return;
    }
    std::unique_lock<std::mutex> lock(s_context->mutex);
    //lock.
    if (!lock.owns_lock())
    {
        MyDebug::Log("!lock.owns_lock()");
        return;
    }
    ...
    {
        MyDebug::Log("MyDebug::OnRenderEvent:: track is not found");
    ...
    {
        MyDebug::Log("MyDebug::VideoStreamRenderEventID::Initialize");
    ...
    {
        // MyDebug::Log("MyDebug::VideoStreamRenderEventID::Encode");
        UnityVideoTrackSource *source = s_context->GetVideoSource(track);
        if (source == nullptr)
            return;
    ...
    {
        MyDebug::Log("MyDebug::VideoStreamRenderEventID::Finalize");
        s_context->FinalizeEncoder(s_mapEncoder[track].get());
        s_mapEncoder.erase(track);

        // Owais:: Call the callback that was registered with "RegisterOnVideoStreamRenderEventFinalize"
        if (OnVideoStreamRenderEventFinalize != nullptr)
            OnVideoStreamRenderEventFinalize();
        return;
    }
    default:
    {
...
extern "C" void UNITY_INTERFACE_EXPORT UNITY_INTERFACE_API RegisterOnVideoStreamRenderEventFinalize(void *track, VideoStreamRenderEventFinalizeCallback callback)
{
    // s_mapEncoder[track]->OnVideoStreamRenderEventFinalize = callback;
    OnVideoStreamRenderEventFinalize = callback;
    return;
}
